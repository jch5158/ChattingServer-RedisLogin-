...
void CLanLoginClient::sendLoginServerLoginRequest(void)
{
	...
	packingLoginServerLoginRequest(dfSERVER_TYPE_CHAT, pMessage);
...
void CLanLoginClient::sendClientLoginSuccess(UINT64 accountNo)
{
	...
	packingClientLoginSuccess(accountNo, dfSERVER_TYPE_CHAT, pMessage);
...
{
	*pMessage << (WORD)en_PACKET_SS_LOGINSERVER_LOGIN << (BYTE)serverType;
